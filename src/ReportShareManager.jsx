...
export default function ReportShareManager({
  ...
  return <div>
    {allowCreate && reportId && <div className="flex flex-col gap-3 border-b border-line bg-canvas p-4 sm:flex-row sm:items-end">
      <label className="block flex-1"><span className="mb-2 block text-xs font-semibold">Optional expiry date</span><input className="field" type="date" value={expiryDate} onChange={(event) => setExpiryDate(event.target.value)} /></label>
    ...
    {message && <p className="border-b border-blue/20 bg-blue/5 px-4 py-3 text-sm text-blue">{message}</p>}
    ...
    {loading ? <p className="p-5 text-sm text-zinc-500">Loading share links…</p> : shares.length ? <div className="divide-y divide-line">{shares.map((share) => {
      ...
      return <article className="grid gap-4 p-4 lg:grid-cols-[1fr_auto]" key={share.id}>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {active && <button className="inline-flex items-center gap-2 border border-red-200 px-3 py-2 text-xs font-semibold text-red-700 hover:bg-red-50" disabled={workingId === `deactivate-${share.id}`} onClick={() => deactivate(share)}><ShieldOff size={14} />Deactivate</button>}
        </div>
      </article>
