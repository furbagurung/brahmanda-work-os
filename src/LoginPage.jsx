...
export default function LoginPage({ onLogin }) {
  ...
  return (
    <main className="grid min-h-screen bg-canvas lg:grid-cols-[minmax(0,1fr)_520px]">
      <section className="hidden border-r border-line bg-white p-12 lg:flex lg:flex-col lg:justify-between">
        <div className="flex items-center gap-3">
          <span className="flex h-9 w-9 items-center justify-center bg-blue text-white"><Command size={19} /></span>
          <span className="text-sm font-bold tracking-tight">BRAHMANDA <span className="text-blue">OS</span></span>
        </div>
        <div className="max-w-xl">
          <span className="text-8xl font-light leading-none text-zinc-100">01</span>
        ...
        <div className="w-full max-w-sm">
          ...
          <form className="mt-8 space-y-5" onSubmit={submit}>
            <label className="block">
              ...
              <span className="flex items-center border border-line bg-white px-3 focus-within:border-blue">
                <Mail size={16} className="shrink-0 text-zinc-400" />
                <input className="w-full px-3 py-3 text-sm outline-none" type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" required placeholder="you@brahmandatech.com" />
              </span>
            </label>
            <label className="block">
              ...
              <span className="flex items-center border border-line bg-white px-3 focus-within:border-blue">
                ...
                <input className="w-full px-3 py-3 text-sm outline-none" type={showPassword ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" required placeholder="Enter password" />
            ...
            {error && <p className="border border-red-200 bg-red-50 px-3 py-2.5 text-sm text-red-700" role="alert">{error}</p>}

            <button className="button-primary w-full py-3" disabled={loading} type="submit">
